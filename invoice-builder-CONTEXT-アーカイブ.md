# invoice-builder CONTEXTアーカイブ（詳細履歴・2026/7/18分割）

> **これはアーカイブです。現行の正は`invoice-builder-CONTEXT.md`（スリム版）。**
> 2026/7/18に、肥大化したCONTEXT（784行）を「仕様サマリー中心のスリム版」と「本ファイル＝セッション別の詳細履歴①〜㊽＋セッション⑫」に分割した。
> スリム版に書ききれない実装経緯・試行錯誤・検証手順の詳細が必要なときだけ、本ファイルの該当セクションをgrepで参照する（全読み不要）。
> 以下は分割時点のCONTEXT全文をそのまま保存したもの。
...
### ㉜ ログイン入口を顧客から隠す（見た目の措置）【index.html・反映済み・実機確認】

- **背景**：顧客が`client.html?id=…`のid部分を消すとルート`/`に到達し、管理画面のログイン画面（「Googleでログイン」ボタン）が見えてしまう。データはルール＋UIガードで守られ実害はないが、顧客に「ログインできる＝不安」と思わせないため入口を隠したいという要望。
- **対応**：index.htmlのログインボタンに`id="googleLoginBtn"`＋`style="display:none"`を付け既定で非表示にし、モジュール先頭で`new URLSearchParams(location.search).has('login')`が真のときだけ表示。あわせて字幕から「Excelテンプレートから」を削除し「請求書メーカー／かんたんにPDF作成」だけに。
- **結果**：顧客がルート`/`を開くとボタン無しの無害な案内のみ。管理者は末尾に`?login`を付けたURL（`https://invoice-builder-iota-lac.vercel.app/?login`）をブックマークして表示・ログインする。実機確認済み。
- **位置づけ**：これは「隠す（obscurity）」措置であってセキュリティ本体ではない。実際の防御は`isAdmin()`ルールと`onAuthStateChanged`のUIガードが担う（＝ボタンが見えても本人以外は入れず、データも取れない）。
...
## 2026/7/13 セッション⑦：フェーズ2「入力バリデーション」対応

> 変更ファイルは **`invoice-builder/index.html`** と **`invoice-builder/client.html`** の2つ（`index.js`未変更＝Cloud Run再デプロイ不要）。GitHubへUploadでVercel自動反映。

### ㉝ 入力バリデーション：特殊文字（XSS/属性崩れ）と極端に長い文字列の対策【index.html＋client.html・要Upload】

**点検結果と修正：**

- **【本丸】index.htmlのクライアント一覧描画が未エスケープだった**：`loadClientList`で`${d.clientName}`を`<td>`に生埋め＋削除ボタンの`onclick`に`'`だけエスケープして直埋めしていた（`"`や`<`を含むクライアント名で属性が壊れる／HTML注入になる。過去の「学び：onclick直埋めリスク」のルール違反が残っていた箇所）。→ **`escHtml()`＋`data-*`方式に全面改修**：クライアント名は`escHtml`で表示、URL/ID/名前は`data-url`/`data-id`/`data-name`属性に入れて`onclick="copyUrl(this, this.dataset.url)"`等で読む。読み込みエラー表示の`${e.message}`も`escHtml`化。
- **client.htmlの残り未エスケープ2箇所を修正**：一括編集カードの発行日`${p.issueDate...}`・締め日`${p.issueDay...}`（issueDateは手入力可のため）を`escHtml`化。明細見出しの`${getColumnLabels()[n]}`（管理者入力の項目名）も単体表・一括表の全12箇所を`escHtml`化。**※PDF送信payload（`columnLabels: getColumnLabels()`）は生のまま**（エスケープするとPDFに実体参照が出るため。表示用と送信用を分ける）。
- **極端に長い文字列：`maxlength`を全入力欄に付与**（従来は全ファイルで0件。Firestoreの1MB文書上限・PDFレイアウト崩れ・貼り付け暴走の予防）：
  - index.html：クライアント名60／会社名60／明細項目名（colName0〜5）各20
  - client.html：取引先名60／請求書番号40（単体・一括とも）／発行日手入力30／明細の日付50・内容150・備考150（単体・一括とも）／自由項目の項目名40・内容300（上段・会社情報共通テンプレとも）／振込先300／自由記入欄（下段）textarea 1000
- **空欄・異常値は既存ガードで問題なしと確認**：クライアント名・取引先名は空欄チェック済み、数値欄は`parseFloat(...)||0`で異常値は0扱い＋`type="number" min="0"`。取引先上限（⑱）も既存。追加対応不要と判断。

**検証**：ホスト側で両HTMLの完全性確認（`</html>`あり、client.html 2199行／index.html 976行、bash側と一致＝末尾切れなし）。`<script type="module">`を抽出して`node --check`＝両ファイルSYNTAX OK。`onclick`へのユーザーデータ直埋めが0件になったことをgrepで確認。**実機確認（Upload後）**：①管理画面のクライアント一覧表示・URLコピー・編集・削除が従来どおり動くこと（data-*化の影響確認・最重要）、②`"`や`<`入りのテスト用クライアント名で一覧が壊れないこと、③顧客ページの取引先追加・明細入力・PDF出力が従来どおり動くこと。

**残り（フェーズ2）**：例外処理（PDF生成失敗・ネットワーク断時のトースト確認）／HTTPS確認。

### ㉞ 例外処理：PDF生成失敗・ネットワーク断のエラー表示を整備【client.html・要Upload】

**点検結果**：Firestore系のcatchは全箇所トースト表示済みで問題なし。PDF系に4つの穴があり修正した。

- **非JSONエラー応答で生の解析エラーが出る**：`generatePDF`の`if(!res.ok){ const err = await res.json(); ... }`は、Cloud Run障害時のHTML/テキスト応答（503等）で`res.json()`自体が`Unexpected token`を投げ、それがそのままトーストに出ていた。→ JSON解析を`try`で包み、ユーザー向けは「サーバーでエラーが発生しました（コード：503）。時間をおいて…」に統一。**サーバーの内部エラー詳細（`err.error`）はトーストに出さずconsole.errorのみ**（「内部エラーを生で見せない」方針）。
- **ネットワーク断で英語の生エラー**：`fetch`自体の失敗（`Failed to fetch`）がそのまま出ていた。→ `fetch`を`try/catch`で包み「サーバーに接続できませんでした。ネットワーク接続をご確認のうえ…」に置換（詳細はconsoleのみ）。
- **【本丸】一括出力の部分失敗がサイレント**：`bulkEditExport`のループ内catchは`console.warn`だけで、一部失敗してもユーザーには「完了！」＋欠けたZIPが渡っていた。→ `failed[]`に取引先名を集計し、(a)**全件失敗**＝ZIPを出さず「一括PDF出力に失敗しました…」（空ZIPで成功と誤解させない）、(b)**部分失敗**＝ZIPは出すが進捗を「完了（成功X件／失敗Y件）」にし「一部のPDFを生成できませんでした（ZIPに含まれていません）：A社、B社」をトースト表示、(c)全成功＝従来どおり「完了！」。
- **PDF成功後の保存失敗で誤メッセージ**：`exportPDF`はPDFダウンロード成功後にFirestore保存が失敗しても「PDF生成に失敗しました」と出て誤解を招いた。→ `pdfDone`フラグで区別し「PDFは出力されましたが、内容の保存に失敗しました：…」に。

**検証**：変更ロジックをスクラッチパッド（`outputs/check-error-handling.js`）に切り出し、nodeスタブハーネスで7ケース（ネットワーク断／非JSON 503／JSON 500の内部詳細非表示／正常／一括の全失敗・部分失敗・全成功）すべて期待どおりを確認。※bashマウントは今回も末尾切れ状態（2178行表示・ホストは2231行で完全）のため、既知ルールどおり切り出し検証方式を採用。**実機確認（Upload後）**：①通常のPDF出力・一括出力が従来どおり動く、②機内モード等でPDF出力→日本語の接続エラートーストが出る（英語の生エラーが出ない）。

**許容（未対応）**：Firestore保存系エラーの`e.message`は英語のFirebase文言が混じるが、接頭辞（「保存に失敗しました：」）が日本語で状況は伝わるため現状維持。必要になれば将来まとめてマッピングする。

### ㉟ HTTPS確認【確認のみ・対応不要】

- `http://invoice-builder-iota-lac.vercel.app/`・`http://invoice-pdf-server-682338340449.asia-northeast1.run.app/`とも、実アクセスで`https://`への自動リダイレクトを確認（Vercel・Cloud Runの標準機能）。コード内参照（`PDF_SERVER`）もhttps。追加対応なし。
- **これでフェーズ2（堅牢性・セキュリティ）の全項目が完了**。㉝㉞の変更ファイル（`index.html`・`client.html`）のUpload＋実機確認が済み次第、フェーズ3（運用・法務・再現性）へ。

---

## 2026/7/13 セッション⑦（続き）：フェーズ3着手・技術系2項目

> フェーズ2完了の実機確認後、フェーズ3の技術系2項目（依存固定の再点検・Google Fonts安定性）を実施。変更ファイルは**すべて`invoice-pdf-server/`**＝`Dockerfile`・`package.json`・`package-lock.json`・`index.js`の4つ。**GitHubへUpload→Actions自動デプロイ→個別・一括PDFの実機確認が必要**（依存・ベースイメージ変更時の必須手順）。

### ㊱ 依存固定の再点検→npm ci化＋express正確固定【invoice-pdf-server・要デプロイ】

- **点検OK**：Node22（Dockerfile `node:22-slim`）／puppeteer `21.11.0`正確固定／`package-lock.json`コミット済み（lockfileVersion 3・191パッケージ・GitHubとミラー一致）／aptのChromium不使用／deploy.yml標準形。
- **注意（再確認）**：GitHubのraw取得でpackage.jsonが旧`^21.0.0`に見えたが、`?cb=`キャッシュ回避で`21.11.0`を確認＝**CDNキャッシュは5日前のコミットでも古いままのことがある**。rawの確認は必ずキャッシュ回避付きで。
- **改善①：Dockerfileを`npm ci --omit=dev`へ**（旧`npm install --omit=dev`）。`npm install`はlockと矛盾があるとlockを書き換えて進んでしまい「lockをコミットした意味」が薄れる。`npm ci`はlockを厳密適用し、矛盾があればビルド失敗で確実に気づける（再現性の本命）。
- **改善②：expressを`4.22.2`に正確固定**（旧`^4.18.2`・lockの実体と同値）。`npm ci`はpackage.jsonとlockの整合を検査するため、**lockの宣言部（`packages[""].dependencies`）も同時に`4.22.2`へ修正**（バージョン解決自体は不変＝インストール内容は1バイトも変わらない）。
- **検証**：両ファイルの宣言一致＋lock実体（express 4.22.2／puppeteer 21.11.0）を確認。lockはホスト側で末尾まで完全（2237行）。※bashマウント経由の`cp`で作った/tmpコピーはまた破損（末尾1バイト余分）＝既知問題の再現。検証・アップロードは必ずホスト側ファイルで。

### ㊲ Google Fonts読込の堅牢化（角印Yuji Syuku）【index.js・要デプロイ】

- **点検**：既存実装（`networkidle0`待ち＋`document.fonts.ready`＋serifフォールバック）は基本堅牢。唯一の弱点＝Google Fontsが「失敗」でなく「無応答（ハング）」の場合、`networkidle0`がナビゲーションタイムアウト既定30秒まで待ち、PDF生成が遅延・失敗し得る。
- **対応**：①`page.setContent(html, { waitUntil:'networkidle0', timeout:15000 })`＋try/catch（打ち切ってもHTML自体は適用済みなので続行してPDF化）。②`fonts.ready`を`Promise.race([document.fonts.ready, 3秒タイマー])`で打ち切り。**どちらも打ち切り時はフォールバック書体で出力継続**＝「フォントが多少違うPDF」は出るが「PDFが出ない」は起きない。
- **検証**：スタブハーネスでタイムアウト時に続行してPDF処理へ到達すること・正常時の挙動不変を確認。`node --check`は変更ブロックの切り出しでOK（マウント末尾切れのため全文チェックは不可、ホスト側は561行で完全）。
- **実機確認（デプロイ後）**：個別PDF・一括ZIPが正常に出ること、角印の書体（Yuji Syuku）が従来どおり出ること。

### ㊳ フェーズ3残り3項目（ロールバック・バックアップ・法務）【2026/7/13完了】

- **ロールバック手順**：`invoice-builder/ロールバック手順.md`新規作成（層別切り分け表／Vercel Instant Rollback／Cloud Run前リビジョン切替／ルール復元／データ復元／戻した後の3点一致チェックリスト。全手順クリックのみ）。UI文言は実画面と要突き合わせ（変わっていたら手順書を修正）。
- **バックアップ体制**：`invoice-builder/バックアップ体制.md`新規作成。管理画面の全体JSONバックアップが**全クライアント＋partnersサブコレクション込みの完全出力**であることをコードで確認。方針＝月1回＋節目、PC＋外部（クラウド/USB）の2箇所保管、直近3世代＋月次保持。GCPエクスポート/PITRは見送り（理由記録済み）。**リストアテストは管理画面（クライアント単位）・顧客ページ（取引先単位）とも実機確認済み（2026/7/13）**。
- **法務**：`利用規約（草案）.md`・`プライバシーポリシー（草案）.md`を作成、本人確認済み（制定日2026/7/13、株式会社キレイット、当社表記、賠償上限＝本サービスの対価として支払った金額の総額（売り切り対応）、サービス終了予告＝原則3か月前、連絡先0313labo@gmail.com）。掲示＝顧客ページリンク＋納品時文書の2本立て。`terms.html`・`privacy.html`（静的・同デザイン・相互リンク付き）を新規作成し、`client.html`に`.page-footer`（利用規約｜プライバシーポリシー、控えめな11px・target=_blank）を追加。**要Upload：client.html・terms.html・privacy.html（invoice-builderリポジトリ→Vercel自動反映）**。実機確認＝顧客ページ下部にリンクが出て両ページが開くこと。※草案はたたき台。必要に応じて専門家確認を推奨。
- **これでフェーズ3（運用・法務・再現性）の全項目が完了**。残るはフェーズ4（リリース当日）＝スモークテスト＋初回納品フロー実測のみ。

---

## 2026/7/14 セッション⑧：同名バグ2件の修正

> 変更ファイルは **`invoice-builder/client.html`** と **`invoice-builder/index.html`** の2つ（`index.js`未変更＝Cloud Run再デプロイ不要）。GitHubへUpload→Vercel自動反映→**実機確認済み**。

### ㊴ 同名取引先の一括PDF出力・一括バックアップでZIP内上書き【client.html・実機確認済み】

- **症状**：同一名の取引先を登録して一括PDF出力すると、PDFが1件分しか出力されない（個別出力は正常）。
- **原因**：ZIP内ファイル名が`請求書_取引先名_日付.pdf`のため、同名取引先だとJSZipに同名で追加され黙って上書きされる。取引先データ自体は`p.id`（UUID）で別管理されており、ファイル名だけの問題。一括バックアップ（`バックアップ_取引先名_日付.json`）にも同じ穴があった。
- **修正**：`bulkEditExport`・`bulkBackupExport`の両方に`usedNames`マップを追加し、同名2件目以降を`請求書_A社(2)_日付.pdf`形式の連番付きにリネーム。`node --check`＋スタブ実行（同名3件＋別名1件→全ユニーク）で検証。

### ㊵ 同名クライアントの点検→復元ドロップダウンにID表示【index.html・実機確認済み】

- **点検結果**：管理画面の登録〜URL発行〜顧客ページ〜PDF出力の流れは、同名クライアントによる上書き・消失バグ**なし**。全処理がUUIDキー＝登録は新規に`crypto.randomUUID()`発行／一覧のURLコピー・編集・削除は`data-id`/`data-url`／発行URLは`?id=UUID`で名前無関係／PDFファイル名は取引先名ベースでクライアント名不使用／全体バックアップは日付名の単一ファイル・復元も`c.id`照合。
- **唯一のリスク（UX）**：「データの復元」のクライアント選択ドロップダウンが名前だけの表示で、同名クライアントだと取り違えて破壊的復元（選択先の取引先を全削除して上書き）をする恐れ→表示を**「クライアント名（ID: 先頭8桁）」**に変更。ID先頭8桁は一覧の「URLをコピー」のURL末尾`?id=`と突き合わせて確認できる。あわせて同行の`escHtml`漏れ（㉝の取り残し）も解消。

### ㊶ 明細の項目名（columns）編集を管理画面→顧客ページへ移管【2026/7/14 セッション⑨・反映済み・実機確認済み】

> 変更ファイル＝`client.html`・`index.html`・`firestore.rules`の3つ（`index.js`未変更＝Cloud Run再デプロイ不要。ラベル/ON/OFFはPDF payload経由のため）。公開順序＝①Firestoreルールをコンソールで公開（先行・無害）→②client.html・index.htmlをUpload。**ルール公開・Upload・実機確認とも完了（2026/7/14）**。

- **背景**：振込先・印鑑・配色は顧客ページの「共通項目」なのに項目名だけ管理画面にあり不整合だった。移管で「PDFの見た目設定は全部顧客ページ」に統一され、納品後の変更もお客様自身で可能に（サポート往復減）。
- **client.html**：「明細の項目名」共通項目カードを配色カードの直後に追加（`buildColumnsEditHTML`＝6項目ラベル入力＋数量/単価/備考の表示チェック＋税率固定行。UIは旧index.html㉑の流用）。保存＝`saveColumnsInfo`が`setDoc(..., { columns, updatedAt }, { merge:true })`。**明細表・一括表・PDFは開くたびに`getColumnLabels()`/`colEnabled()`から再構築されるため、保存時は`currentClient.columns`のメモリ更新だけでよい（再描画処理不要）**。CSS（`.col-row`/`.col-badge`系）も追加。
- **index.html**：項目名カード＋関連JS（COL_DEFS/setColUI/updateColDim/getColumnsFromUI）を削除。**保存時は既存`columns`を保持**（setDoc全上書きで消えるのを防ぐ・㊾会社情報保持と同じパターン）。新規クライアントには`DEFAULT_COLUMNS`（全表示・デフォルト名）を書き込む。editClientの`setColUI`呼び出しも削除。
- **firestore.rules**：未認証updateの`hasOnly`に`'columns'`を追加（`['companyInfo','projectFields','updatedAt','columns']`）。社名・clientName・clientId改変禁止は不変。**コンソール貼り付け→公開が必要**（このファイルからの自動デプロイは無い）。リスク上の新規性なし（秘密URLを知る顧客が自分の項目名を変えられるだけ＝振込先等と同レベル）。
- **検証**：両HTMLのモジュール抽出で`node --check`＝SYNTAX OK、divタグ収支一致、index.htmlに旧関数の取り残し0件をgrep確認。バックアップ互換＝全体バックアップはdoc全体を出力するため`columns`も含まれ影響なし。
- **実機確認済み（2026/7/14）**：顧客ページの項目名カード表示・保存、見出し変更・OFF列非表示の反映を確認OK。
...
## 2026/7/15 セッション⑩：PDFタイトルの請求書/ご請求書切替・会社情報の＋〒ボタン

> 変更ファイルは **`invoice-pdf-server/index.js`** と **`invoice-builder/client.html`** の2つ（`index.html`未変更）。index.js＝Cloud Run再デプロイ要、client.html＝Vercel自動反映。**GitHubへUpload→デプロイ→実機確認まで完了（2026/7/15）**。firestore.rules変更なし（`invoiceTitle`は`companyInfo`配下のネストで既存の未認証update許可範囲内）。

### ㊷ PDFタイトルを「請求書／ご請求書」から選択【index.js＋client.html・実機確認済み】

- **背景**：請求書PDFの一番上のタイトルを、丁寧な「ご請求書」表記にも切替えたいという要望。クライアント単位の共通項目として実装。
- **データ形式**：`companyInfo.invoiceTitle = '請求書' | 'ご請求書'`（既定`請求書`）。他のPDF見た目設定（会社情報・振込先・印鑑・配色）と同じ「共通項目」パターン。
- **client.html**：会社情報カードの直前に「PDFのタイトル」共通項目カードを新設（`buildTitleEditHTML`＝請求書/ご請求書のラジオ＋保存ボタン、`saveTitleInfo`＝`{companyInfo, updatedAt}`のmerge保存＋`currentClient.companyInfo`メモリ更新）。個別PDFは`generatePDF`が`currentClient.companyInfo`を送るため保存後の値がそのまま反映。
- **index.js**：`const invoiceTitle = (company.invoiceTitle === 'ご請求書') ? 'ご請求書' : '請求書'; const title = docType === '見積書' ? '見積書' : invoiceTitle;`。タイトル表示は既存の`title.split('').join('　')`（全角スペース区切り）でそのまま「ご　請　求　書」になる。
- **振込先の表示条件を変更**：従来`title === '請求書'`で振込先ブロックを出していたが、「ご請求書」だと振込先が消えてしまうため`docType !== '見積書'`に変更（請求書・ご請求書の両方で振込先が出て、見積書のときだけ出さない、という本来の意図を維持）。
- **実機確認済み（2026/7/15）**：ご請求書に切替→タイトル反映＋振込先表示OK。

### ㊸ 会社情報の編集に「＋〒」プリセットボタン【index.js＋client.html・実機確認済み】

- **背景**：郵便番号を入れやすくするプリセット。PDFでは郵便番号マーク「〒」の後にコロンを出さない（`〒100-0001`が自然）ようにしたい。
- **client.html**：`buildCompanyEditHTML`のプリセットボタン群に`＋〒`を追加（`addCompanyCustomField('〒')`＝ラベル`〒`の項目を挿入）。項目自体は通常の`{label:'〒', value}`で、保存・並び替え・レガシー移行は既存ロジックのまま（client.html側に特別扱いなし）。
- **index.js**：会社情報描画（`companyFieldsHtml`）で**ラベルが`〒`のときだけコロンを付けず**`〒`＋値で描画（`const head = label === '〒' ? '〒' : (label ? label + '：' : '');`）。他の項目は従来どおり`項目名：値`、ラベル空欄は値のみ。
- **実機確認済み（2026/7/15）**：`＋〒`で郵便番号を入れ→PDFが`〒100-0001`（コロンなし）で表示OK。

### ㊹ 自由項目上段（project-info）で半角数字が上にずれる【index.js・実機確認済み】

- **症状**：自由項目記入欄（上段）の値に半角数字（例`011117`）を入れると、ラベル（全角）に対して値だけ少し上にずれて印字される。全角（`０１１１１７`）は正常。
- **原因**：`.project-field-row`が`display:flex`で交差軸の揃え未指定＝既定`align-items:stretch`。ラベルと値のスパンがそれぞれの行ボックス上端に配置され、半角数字はフォント（ASCII）の上下メトリクスが全角（CJK）と異なるため値だけ上に浮いて見えた。全角同士は同じCJKメトリクスなので揃って見えていた。
- **修正**：`.project-field-row`に`align-items: baseline;`を追加。ラベルと値をベースラインで揃え、半角数字のずれを解消。全角はもともと揃っていたので見た目不変。変更は`index.js`1ファイルのみ（Cloud Run再デプロイ要）。
- **学び**：`display:flex`でラベル＋値を横並びにする箇所は、和文（全角）と欧文/数字（半角）が混在すると既定`stretch`で縦位置がずれる。`align-items:baseline`で揃える。※`companyFieldsHtml`（会社情報）は`<div>`ベースでflex横並びではないため同種の問題は出ない。
- **検証メモ**：node --checkは`invoice-pdf-server/index.js`でbashマウント末尾切れ（562行目の文字列が途中で切れて見える）により失敗表示になったが、ホスト側の実ファイルは正常。今回の変更はテンプレート文字列内へのCSS 1行追加＝構文非影響のため実害なし（既知のマウント破損対処どおり、ホスト側Readで完全性を確認）。

### ㊺ PDFの文字サイズを全体的に拡大（本文まわり）【index.js・実機確認済み】

- **背景**：印刷すると文字が小さく感じる、という相談。前提整理として、①PDFの文字サイズは絶対値で決まり印刷で縮まない、②画面表示は実寸より拡大されがちなので印刷（実寸）が小さく感じる、③印刷ダイアログの「ページに合わせる」等で数％縮む場合がある、を確認。参考換算＝エクセルのMSゴシック11pt＝約14.7px（96dpi）。従来の本文13px（≒9.75pt）はやや小さめだったため、本人指定の値へ引き上げた。
- **変更（14項目・すべて`index.js`のCSS）**：明細見出し`th` 11→14px／明細本文`td` 13→15px／小計・消費税行`.total-row` 12.5→15px／合計最終行（インライン）15→16px／会社名`.company-name` 14→15px／会社情報の各項目`.company-side` 12→14px／請求書番号`.invoice-no-line` 12→14px／請求日`.issue-date-line` 12→14px／敬称`.client-name span` 13→14px／挨拶文`.greeting` 12→13px／税込合計ラベル`.total-highlight-label` 13→14px／自由項目上段`.project-field-row` 13→14px／自由記入欄下段`.remarks-lower` 12.5→14px／振込先見出し`.bank-info-title` 13.2→14px。**据え置き**＝タイトル`h1` 23px／取引先名`.client-name` 19px／税込合計の数字`.total-highlight-value` 24px／振込先本文`.bank-info-body` 16.2px／body基準13px。
- **実機確認済み（山田デザイン事務所テストデータ）**：明細（日付〜備考）崩れなし、内容欄6行折り返しでも2ページ目まで影響なし。見出しの1行収まりもOK。
- **注意（列幅）**：明細列幅（15/31/6/14/14/6/14）は旧・小フォントでピクセル実測した値。今回見出し14px・本文15pxに上げたが実機で1行維持を確認済み。今後さらに上げる場合は列幅の再確認が必要。複数ページの1ページ収容行数はフォント拡大で従来目安より減る。

### ㊻ 長い入力時にPDF全体が縮小する挙動＝Chromiumの自動縮小（仕様として現状維持）

- **症状**：異常に長い住所・自由項目（区切りのない長い文字列）を入れると、PDF全体（文字含む）が小さくなる。
- **原因（＝自作のリサイズ機能ではない）**：`page.pdf`は倍率1（等倍、`scale`未指定）。1行が折り返せずA4幅を超えると、**ChromiumのPDFエンジンがページ全体を紙幅に収めるため自動縮小**する（Chrome印刷の「ページに合わせる」相当の挙動）。明細`td/th`には`overflow-wrap:anywhere`があり折り返すが、**会社情報の各項目（`.company-side`配下）と自由項目の値（`.project-field-value`）には折り返し指定が無い**うえ、自由項目は横並び(flex)で値スパンが既定では縮まないため、区切りのない超長文が幅をはみ出す→全体縮小になる。日本語の通常文は文字間で折り返すため縮まない（縮むのは半角英数字の長い連続や区切り無し文字列）。
- **判断＝現状維持（折り返し対応は入れない）**：長い住所（ビル名等）は「項目を分けて入力」で回避できるため、本人希望で折り返しCSS（会社情報・自由項目値への`overflow-wrap`/`word-break`＋flex値の`min-width:0`）は今回入れない。将来もし対応するなら上記CSSで「全体縮小→枠内折り返し」に変えられる（文字サイズ一定を保てる）。
- **学び**：Puppeteer `page.pdf`は`scale:1`でも、折り返せない幅超えコンテンツがあるとChromiumが全体を縮小する。等倍で出したい要素は、幅を超えうる自由入力欄に`overflow-wrap`/`word-break`（flexなら`min-width:0`も）を付けて折り返し可能にしておく。

### 共通項目の取引先ごとの出し分けについて（設計メモ）

- **質問**：クライアントが取引先ごとに共通項目（会社情報・振込先・印鑑・配色・タイトル・明細項目名）を変えて、個別にPDFを出せるか。
- **回答＝可能**。全カード共通で、個別PDF出力（`generatePDF`）は出力時点の`currentClient.companyInfo`／`currentClient.columns`を送るため、「内容変更→そのカードの保存→個別出力」を繰り返せば取引先ごとに異なるPDFが作れる（明細項目で確認済みの挙動と同一）。各保存関数（saveCompanyInfo/saveBankInfo/saveSealInfo/saveThemeInfo/saveTitleInfo/saveColumnsInfo）がいずれも`currentClient.companyInfo`（columnsは`currentClient.columns`）をメモリ更新するため。
- **設計上の性質（注意）**：共通項目はクライアント文書に1組だけ保存＝**取引先ごとに記憶されない**。保存のたび上書きで最後に保存した値が残る（A出力→Bに変えて保存→後日A再出力は、戻さない限りB用の値で出る）。保存ボタンを押さないと出力に反映されない。**一括出力ZIPは全取引先に同じ共通項目を適用**するため、取引先ごとに変えられるのは個別出力時のみ。取引先ごとに固定で覚えさせたい場合は、その項目を取引先単位（partners配下）へ移す設計変更が必要（今回は現状維持で合意）。

---

## 2026/7/16 セッション⑪：ファビコン設置＋ココナラ出品準備（顧客マニュアル・デモデータ）

### ㊼ ファビコン（favicon）を設置【index.html・client.html・terms.html・privacy.html＋アイコン7ファイル・反映済み】

- **デザイン**：顧客向け操作マニュアルの表紙アイコン（白い書類＋青ヘッダー＋グレー明細線＋朱色の角印）を、**ネイビー（#2C3E50）角丸の下地**に載せたアプリアイコン風。ブランド色（ネイビー／ブルー#6699CC／朱色#C0392B）で統一。
- **生成ファイル（invoice-builderリポジトリのルートに配置）**：
  - `favicon.ico`（16/32/48px内包・旧ブラウザ／既定探索用）
  - `favicon.svg`（最新ブラウザ用・ベクター）
  - `favicon-16x16.png`（タブ実サイズ・視認性のため線を減らした簡略版）／`favicon-32x32.png`（高解像度タブ等）
  - `apple-touch-icon.png`（180px・iOSホーム画面用・角丸はOSが付与）
  - `icon-192.png`／`icon-512.png`（Android・PWA用。manifest.jsonは現状なしのため将来用）
  - ※16pxのみ簡略版、48px以上は表紙と同じ細線版。
- **HTML変更**：4ファイルの`<head>`（viewport行の直後）に`<link rel="icon">`系4行＋`<link rel="apple-touch-icon">`を追加。パスはルート絶対（`/favicon.ico`等）。
- **反映**：GitHubへUpload→Vercel自動反映、ブラウザタブでの表示を確認済み（設定完了）。
- **メモ**：将来PWA化（ホーム画面追加を本格対応）する場合は`manifest.json`を追加し、iconsに192/512を登録する。自動生成角印は社名に【】（）等の括弧・記号があると崩れる既知の性質があり、デモの発行元名を括弧なし（デモサンプル工務店）にして回避した。

### ㊽ ココナラ出品準備の成果物（顧客マニュアル・デモデータ）※詳細は接続フォルダ`ココナラ出品/`

> 出品準備の進捗の正は`ココナラ出品/00-進捗メモ.md`。ここには開発に関わる要点のみ記録。

- **顧客向け操作マニュアル（PDF・全24ページ）**：`ココナラ出品/顧客向け操作マニュアル_v7.pdf`（編集用HTML＝`顧客向け操作マニュアル_編集用.html`。実スクショをbase64で埋め込み済み・自己完結）。※旧v6は最終24ページの本文が途中で切れた欠陥版（編集用HTML末尾未完のままPDF化）。2026/7/18にHTML補完→v7再生成。納品はv7を使用。client.htmlの操作を「最初の設定（初回だけ）→毎月の作業→そのほか」の順で1操作1ページ・実スクショ入りで解説。重要方針として「**このツールは各取引先の前回分のみ保存／過去分は顧客がPDFまたはバックアップJSONで自己管理**」を明記。サポート窓口は**ココナラのトークルームに一本化**（メール記載は外部誘導＝規約違反リスクのため不可。事業者連絡先としての規約類への記載は別扱いで可）。
- **デモサイト用データ**：`ココナラ出品/デモデータ/`。発行元＝**デモサンプル工務店**（括弧なし＝自動角印が崩れない）、取引先3件（【デモ】サンプル建設株式会社／【デモ】サンプル商店＝8%・10%混在で税率区分を提示／【デモ】山田太郎（個人））。取引先の明細は**ワンクリック復元用JSON**（`リセット用_*.json`＝`invoice-builder-partner-backup`形式）で投入。共通項目（会社情報・振込先・印鑑・配色）は手入力（値は`デモサイト構築ガイド.md`に記載）。デモ作成は**フェーズ4スモークテストを兼ねる**。運用＝URLは購入前相談者にのみトークルームで渡す（公開は書き換え/削除リスクのため不可）、荒らされたら同JSONで復元→内容保存でリセット。

---

## 2026/7/18 セッション⑫：ココナラ出品を公開（開発への影響のみ記録）

> このセッションはコード変更なし（client.html・index.html・index.js・firestore.rulesすべて未変更）。作業はすべてココナラ出品関連＝**詳細は`ココナラ出品/00-進捗メモ.md`（出品の正）を参照**。

- **【マイルストーン】ココナラ出品を公開（2026/7/18）**：出品URL＝ https://coconala.com/services/4311730 。本人確認・インボイス登録完了済み。**開発側の残＝フェーズ4（スモークテスト・納品フロー実測）を初回注文後・納品前に必ず実施**（デモ構築ガイドSTEP5が兼用）。
- 顧客向け操作マニュアルをv7に修正（v6は最終ページ欠け・上記㊽参照）。
- デモ運用の保険として、デモクライアントへ`maxPartners`（3〜5）のFirebaseコンソール設定を推奨（⑱の機能を利用。設定はTsuyoshiさん任意）。
- 補助金調査の結論（参考）：現行の2万円売り切りモデルでは国（デジタル化・AI導入補助金）・東京都（デジタル導入促進補助）とも実質対象外。「補助金対象」を謳う宣伝は不可。
...
>
> **【マイルストーン更新・2026/7/13 セッション⑦】フェーズ2完了。** 入力バリデーション（㉝＝index.html一覧のescHtml＋data-*化・全入力欄maxlength）と例外処理（㉞＝PDFエラーの日本語化・一括出力の部分失敗通知・保存失敗の区別）を実装、HTTPS（㉟）は確認のみでOK。変更は`index.html`・`client.html`の2ファイル（`index.js`未変更）。**次回の起点：㉝㉞のUpload＋実機確認の完了報告を受けてフェーズ3（運用・法務・再現性）へ**＝依存固定の再点検／Google Fonts読込安定性／利用規約・プライバシーポリシー／Firestoreバックアップ体制／ロールバック手順の明文化。
>
> **【マイルストーン更新・2026/7/13 ③】フェーズ3完了（㊳）→ フェーズ4は「初回納品前」に読み替えて後回しにし、ココナラ出品準備を完了。** 出品関連の資料・決定事項・残作業は**`見積請求PWA開発/ココナラ出品/00-進捗メモ.md`を正**として参照（競合調査・価格20,000円スタート・プロフィール・出品本文・画像素材4枚まで作成済み。Tsuyoshiさんが登録作業中）。開発側の残りはフェーズ4（スモークテスト・納品フロー実測＝初回注文後、納品前に必ず実施）のみ。
>
> **【マイルストーン更新・2026/7/13 同日続き】フェーズ3の技術系2項目（㊱依存固定＝npm ci化・express正確固定／㊲Google Fontsハング対策）を実施。** `invoice-pdf-server`の4ファイル（Dockerfile・package.json・package-lock.json・index.js）をUpload→Actions自動デプロイ→**個別・一括PDFの実機確認待ち**。フェーズ3の残り＝法務（利用規約・プライバシーポリシー）／Firestoreバックアップ体制／ロールバック手順の明文化。
>
> **【マイルストーン更新・2026/7/14 セッション⑧】同名バグ2件を修正・実機確認済み（㊴同名取引先のZIP内上書き＝client.html／㊵復元ドロップダウンのID表示＝index.html）。** 開発側の残りは引き続きフェーズ4（スモークテスト・納品フロー実測＝初回注文後、納品前に必ず実施）のみ。
>
> **【マイルストーン更新・2026/7/14 セッション⑨】明細の項目名（columns）編集を管理画面→顧客ページへ移管（㊶）・ルール公開＋Upload＋実機確認まで完了。** 変更＝client.html・index.html・firestore.rulesの3ファイル（index.js未変更）。これで共通項目（会社情報・振込先・印鑑・配色・明細の項目名）はすべて顧客ページに統一。あわせて**納品準備2点を作成**：①ヒアリングシート（`ココナラ出品/04-ヒアリングシート.md`＝トークルーム貼り付け用Q1〜Q8。必須は会社名のみ・残りは既定値で仮設定の運用）、②印影透過ツール（`ココナラ出品/05-印影透過ツール.py`＝印影のスマホ写真→朱色透過PNG。redness方式・サンプル検証済み。外部サイト不使用の方針で確定、お客様にはQ3で「透過加工はこちらで行います」と案内）。開発側の残りは引き続きフェーズ4（スモークテスト・納品フロー実測＝初回注文後、納品前に必ず実施）のみ。次の納品準備候補：操作マニュアル（顧客向けPDF）／デモ用クライアント＋リセット手順／納品メッセージテンプレ／納品作業チェックリスト。
>
> **【マイルストーン更新・2026/7/15 セッション⑩】PDFタイトルの請求書/ご請求書切替（㊷）と会社情報の＋〒ボタン（㊸）を追加・実機確認済み。さらに自由項目上段の半角数字ずれ修正（㊹・実機確認済み）、PDF文字サイズの全体拡大14項目（㊺・実機確認済み）、長い入力時のChromium自動縮小は仕様として現状維持（㊻・折り返しCSSは本人希望で入れず、長い住所は項目分割で回避）。** 変更＝`invoice-pdf-server/index.js`・`invoice-builder/client.html`の2ファイル（`index.html`・`firestore.rules`未変更。㊹㊺は`index.js`のみ）。あわせて共通項目の取引先ごとの出し分け可否を整理（全カードで個別出力時に可能・ただしクライアント単位保存で取引先ごとの記憶はしない＝現状維持で合意）。開発側の残りは引き続きフェーズ4（スモークテスト・納品フロー実測＝初回注文後、納品前に必ず実施）のみ。
...
7-2. **【保留・2026/7/13】Cloud Run自動デプロイの高速化**：現状は`--source .`方式で毎回フルビルド（apt約35個＋PuppeteerのChromium約150MB DL）のため約13分かかる。対応方針は合意済み＝(a)GitHub Actions側でdockerビルド（レイヤーキャッシュ付き）→Artifact Registryへプッシュ→`--image`デプロイ（**index.jsのみの変更なら2〜4分**になる見込み。特にコード変更時の高速化を優先）、(b)`paths:`フィルタで不要なデプロイ自体を減らす。必要作業＝Artifact Registryリポジトリ作成（1回・コンソール）＋deploy.yml書き換え。リリース優先のため後回しと判断。
...
- **JSZipは同名ファイルを黙って上書きする**：`zip.file(同じ名前, ...)`はエラーにならず後勝ちで置き換わる。ユーザー入力由来の名前（取引先名等）をZIP内ファイル名に使う箇所は`usedNames`マップ等で連番リネームして一意化する。同様に、名前を選択肢のラベルに使うUI（復元ドロップダウン等）も「重複したらどうなるか」を必ず点検する（IDの一部を併記して区別）。
...
- **セッション開始の合言葉(2026/7/18更新・用途別)**：
  - **開発作業**(コード変更・不具合対応)：「デスクトップの見積請求PWA開発フォルダを接続して、invoice-builder-CONTEXT.mdとdeveloper-profile-updated.mdを読み込んで」(従来通り)
  - **ココナラ関係のみ**(出品改善・相談対応・納品準備)：「デスクトップの見積請求PWA開発フォルダを接続して、ココナラ出品/00-進捗メモ.mdを読み込んで」— **CONTEXTの全読みは不要**(約770行と重いため。技術情報が必要になった時だけ該当箇所をgrepで部分参照)
  - **初回注文の納品対応**：00-進捗メモ＋`invoice-builder/release-roadmap.md`(フェーズ4)を読む
