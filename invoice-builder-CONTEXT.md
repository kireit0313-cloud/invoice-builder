...
- **顧客向け操作マニュアル（PDF・全24ページ）**：`ココナラ出品/顧客向け操作マニュアル_v6.pdf`（編集用HTML＝`顧客向け操作マニュアル_編集用.html`。実スクショをbase64で埋め込み済み・自己完結）。client.htmlの操作を「最初の設定（初回だけ）→毎月の作業→そのほか」の順で1操作1ページ・実スクショ入りで解説。重要方針として「**このツールは各取引先の前回分のみ保存／過去分は顧客がPDFまたはバックアップJSONで自己管理**」を明記。サポート窓口は**ココナラのトークルームに一本化**（メール記載は外部誘導＝規約違反リスクのため不可。事業者連絡先としての規約類への記載は別扱いで可）。
