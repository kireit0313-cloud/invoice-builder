...
>
> **【マイルストーン更新・2026/7/15 セッション⑩】PDFタイトルの請求書/ご請求書切替（㊷）と会社情報の＋〒ボタン（㊸）を追加・実機確認済み。** 変更＝`invoice-pdf-server/index.js`・`invoice-builder/client.html`の2ファイル（`index.html`・`firestore.rules`未変更）。あわせて共通項目の取引先ごとの出し分け可否を整理（全カードで個別出力時に可能・ただしクライアント単位保存で取引先ごとの記憶はしない＝現状維持で合意）。開発側の残りは引き続きフェーズ4（スモークテスト・納品フロー実測＝初回注文後、納品前に必ず実施）のみ。
