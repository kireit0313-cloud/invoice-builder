...
- **`index.js`の2フォルダ同時書き込み ★2026/7/9導入**：Cloud Runのデプロイは`C:\Users\fokar\invoice-pdf-server\`(デプロイ元フォルダ)を使うため、`index.js`を編集するときはClaudeが**ミラーとデプロイ元フォルダの両方へ同じ内容を書き込む**。これによりTsuyoshiは手動コピー不要で、「GitHubへコミット」＋「ターミナルで`gcloud run deploy`」だけで済む。
  - 書き込み先(2箇所): `C:\Users\fokar\Desktop\見積請求PWA開発\invoice-pdf-server\index.js`(ミラー) と `C:\Users\fokar\invoice-pdf-server\index.js`(デプロイ元)。
  - このためには**セッション開始時に両フォルダを接続する**必要がある(下記「合言葉」参照)。デプロイ元フォルダが未接続のときは`mcp__cowork__request_cowork_directory`で`C:\Users\fokar\invoice-pdf-server`の接続を要求する。
  - ※`index.js`以外(`Dockerfile`等)を変更したときも同様に両フォルダへ反映する。
...
- **セッション開始の合言葉**：「デスクトップの見積請求PWA開発フォルダと、invoice-pdf-serverフォルダ(`C:\Users\fokar\invoice-pdf-server`)を接続して、invoice-builder-CONTEXT.mdとdeveloper-profile-updated.mdを読み込んで」
  - 補足：フォルダ接続はセッションごとに毎回必要（前回接続しても新セッションには引き継がれない。永続化する設定は現状なし）。合言葉に「フォルダを接続して」を含めると、接続要求→承認→読み込みが一続きで進む。
  - **2フォルダ接続の理由**：`index.js`をミラーとデプロイ元(`C:\Users\fokar\invoice-pdf-server\`)の両方へ自動書き込みするため（上記「`index.js`の2フォルダ同時書き込み」参照）。`index.js`をいじらない日は`見積請求PWA開発`だけの接続でもよいが、迷ったら両方つなぐのが安全。
