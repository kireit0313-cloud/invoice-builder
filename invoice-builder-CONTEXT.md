...
>
> **【マイルストーン更新・2026/7/12】フェーズ1（機能E2E）ほぼ完了。** テスト1〜7合格、テスト8は前回確認済み。途中で3件修正（㉖日付ずれ／㉗戻るボタン位置／㉘2ページ下線かぶり、すべて反映済み）。**次回セッションの起点：フェーズ2（堅牢性・セキュリティ）**。最優先で㉙のSafariログイン修正（Vercel rewriteで認証を同一ドメイン化）、続けてFirestoreルールの権限境界・入力バリデーション・PDF生成失敗時のトースト・認証（管理画面アクセス制限）。進捗の正は`release-roadmap.md`。
