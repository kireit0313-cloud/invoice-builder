...
>
> **【マイルストーン更新・2026/7/12 セッション⑥】フェーズ2に着手。㉙のSafari管理ログインは「PC（Chrome/Edge）運用で確定・対応不要」と判断（詳細は㉚）。** 中継方式はポップアップ方式に不適合と判明しロールバック済み、Chrome/Edge復旧を実機確認。顧客画面はログイン不要でSafari無影響。**次回の起点：フェーズ2の残り**＝Firestoreルールの権限境界／入力バリデーション（空欄・異常値・特殊文字・`onclick`のdata-*化再点検）／PDF生成失敗・ネットワーク断時のトースト（内部エラーを生で見せない）／認証（承認済みドメイン・管理画面アクセス制限）／HTTPS確認。進捗の正は`release-roadmap.md`。
